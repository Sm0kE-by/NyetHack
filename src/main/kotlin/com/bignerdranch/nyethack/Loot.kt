...
//K задают имя ключей (key), V — значения (value), E — элементы (element) и R — результаты (result)
//Если используют обобщения, которые не подходят под эти категории, обычно применяют обозначения T, U и V
        ...
        fun random(): LootBox<Loot> = LootBox(
            contents = when (Random.nextInt(1..100))
            {
                ...
                in 31..50 -> Fedora("ordinary fedora", 10)
                    ...
                    Gemstones(Random.nextInt(50..100))
            }
        )
    }
}
